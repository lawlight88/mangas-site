                    ...
                    @if (is_null($id_comment_edit))
                        <div class="text-end">
                            <form action="{{ route('comment.delete', $comment->id) }}" method="post">
                                @method('delete')
                                @csrf
                                <a class="text-light btn btn-lg fa-lg d-inline-block" href="{{ route('app.manga.view', ['id' => $manga->id, 'chapter_order' => $chapter_order, 'page_order' => $page->order, 'id_comment_edit' => $comment->id]) . "#c_$comment->id" }}"><i class="fas fa-edit"></i></a>
